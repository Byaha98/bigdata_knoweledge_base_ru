...
import { FilePath, FullSlug, QUARTZ, slugifyFilePath, transliterateForPath, canonicalPathForUrl } from "../util/path"
  ...
  return async (processor: QuartzMdProcessor) => {
    ...
    for (const fp of fps) {
      try {
        ...
        file.data.slug = canonicalPathForUrl(transliterateForPath(slugifyFilePath(file.data.relativePath))) as FullSlug
