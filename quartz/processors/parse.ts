...
import { FilePath, FullSlug, QUARTZ, slugifyFilePath, transliterateForPath } from "../util/path"
  ...
  return async (processor: QuartzMdProcessor) => {
    ...
    for (const fp of fps) {
      try {
        ...
        file.data.slug = transliterateForPath(slugifyFilePath(file.data.relativePath)) as FullSlug
